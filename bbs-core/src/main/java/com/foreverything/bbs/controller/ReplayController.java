...
public class ReplayController {
    ...
    public String replaySomething( HttpServletRequest request){
        ...
        int fromID=Integer.valueOf(request.getParameter("id"));
        replay.setFromId(fromID);
        replay.setContent(request.getParameter("content"));
        replay.setFromType((String) request.getParameter("type"));
        replay.setUserId(Integer.parseInt(request.getSession().getAttribute("userId").toString()));
        System.out.println("获取的评论为："+replay);
        int id=replayService.insertReplay(replay);
        if (id>0){
